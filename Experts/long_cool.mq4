...
#define long_cool 040122

extern double lot = .25;
extern short hour_begin = 10, hour_end = 15, timeframe = 240, stop_loss = 75, limit_entry = 30;
extern bool time = false, random = false, stops = true, limits = false, weekend_breaker = false;
extern short long_chart = 240, mid_chart = 60, short_chart = 240, MA_fast = 21, MA_slow = 55;
extern short ma_method = 0;
...
void OnDeinit(const int reason)
  {
   Alert("long_cool.ex4 deinitialized");
  }
   ...
   if(weekend_breaker)        //breaker is for testing only
     {
      if(!trade_allowed && !paper_buy) 
        {
         if(price_0 > price_1 && price_1 > price_2  && (fastMA_short_1 + fastMA_short_2 + fastMA_short_3)/3 > (slowMA_short_1 + slowMA_short_2 + slowMA_short_3)/3)               //paper buy
           {
            paper_buy = true;
      ...
      if(paper_buy) 
        {
         if(price_1 < price_2  && (fastMA_short_1 + fastMA_short_2 + fastMA_short_3)/3 < (slowMA_short_1 + slowMA_short_2 + slowMA_short_3)/3)                                    //paper close
           {
            paper_buy = false;
      ...
      if(!trade_allowed && !paper_sell) 
        {
         if(price_0 < price_1 && price_1 < price_2 && (fastMA_short_1 + fastMA_short_2 + fastMA_short_3)/3 < (slowMA_short_1 + slowMA_short_2 + slowMA_short_3)/3)               //paper sell
           {
            paper_sell = true;
         ...
         if(!random || MarketInfo(Symbol(),MODE_TIME) >= (iTime(Symbol(),timeframe,0) + random_trade_time))
           {
            if(!time || (Hour() >= hour_begin && Hour() <= hour_end)) 
              {
               if(price_0 > price_1 && price_1 > price_2  && (fastMA_short_1 + fastMA_short_2 + fastMA_short_3)/3 > (slowMA_short_1 + slowMA_short_2 + slowMA_short_3)/3)
                 {
                  if(limits) 
                    { 
                  ...
                  buy = OrderSend(Symbol(),buy_order,lot,buy_limit,0,stop_trade,0,NULL,long_cool,0,clrNONE);
                 }
               if(price_0 < price_1 && price_1 < price_2 && (fastMA_short_1 + fastMA_short_2 + fastMA_short_3)/3 < (slowMA_short_1 + slowMA_short_2 + slowMA_short_3)/3)
                 {
                  if(limits)
                  ...
                  sell = OrderSend(Symbol(),sell_order,lot,sell_limit,0,stop_trade,0,NULL,long_cool,0,clrNONE);
                 }
              }
           }
        }
      ...
      for(i = 0; i < OrdersTotal(); i++)
        {
         if(OrderSelect(i,SELECT_BY_POS,MODE_TRADES))
           {
            if(OrderSymbol() == Symbol() && OrderMagicNumber() == long_cool)
              {
               if(OrderType() == OP_BUY && price_1 < price_2  && (fastMA_short_1 + fastMA_short_2 + fastMA_short_3)/3 < (slowMA_short_1 + slowMA_short_2 + slowMA_short_3)/3)
                 {
               ...
               if(OrderType() == OP_SELL && price_1 > price_2  && (fastMA_short_1 + fastMA_short_2 + fastMA_short_3)/3 > (slowMA_short_1 + slowMA_short_2 + slowMA_short_3)/3)
                 {
