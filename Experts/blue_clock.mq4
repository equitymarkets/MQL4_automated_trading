...
extern short what_minute = 0, smooth_price_period = 2, fast_period = 15, slow_period = 30, signal_period = 5;
int i;
double pips;
               ...
               if(buyer)
                 {
                  ...
                  buy = OrderSend(Symbol(),buy_order,lot,buy_limit,0,stop_trade,0,NULL,blueClock,0,clrNONE);
                 }
               if(seller)
                 {
                  if(limits)
                  ...
                  sell = OrderSend(Symbol(),sell_order,lot,sell_limit,0,stop_trade,0,NULL,blueClock,0,clrNONE);
                 }
              }
           }
        }
      ...
      for(i = 0; i < OrdersTotal(); i++)
        {
         if(OrderSelect(i,SELECT_BY_POS,MODE_TRADES))
           {
            if(OrderSymbol() == Symbol() && OrderMagicNumber() == blueClock)
              {
