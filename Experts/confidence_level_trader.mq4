...
//confidence / gate / size selector
//build confidence level as lot size selector 
...
extern short what_minute = 4, array_lookback_period = 100;
extern double lot = .5;
extern short ATR_timeframe = 5, OBV_timeframe = 60, ATR_period = 13, OBV_break = 10000, ATV_break = 600;
...
void OnTick()
  {
   ...
   double ATR_0 = iATR(Symbol(),ATR_timeframe,14,0);
   double ATR_1 = iATR(Symbol(),ATR_timeframe,14,1);
   double ATR_2 = iATR(Symbol(),ATR_timeframe,14,2);
   
   int OBV_0 = iOBV(Symbol(),OBV_timeframe,PRICE_CLOSE,0);
   int OBV_1 = iOBV(Symbol(),OBV_timeframe,PRICE_CLOSE,1);
   int OBV_2 = iOBV(Symbol(),OBV_timeframe,PRICE_CLOSE,2);
   
   double stop_trade = 0, buy_limit = Ask, sell_limit = Bid;
   //atr for confidence 
   //momentum, oscillator, and volume for gate
   //levels of each for lot selection via confidence_level
   static short confidence_level = 0; 
   
   if(Minute() >= what_minute && (ATR_0 + ATR_1)/2 > ATR_2) 
     {
      bool confidence = true;
     }
     
   if((OBV_0 + OBV_1)/2 > OBV_2 && OBV_1 > OBV_break && price_0 > price_1 && price_1 > price_2) bool buyer = true;
   if((OBV_0 + OBV_1)/2 < OBV_2 && OBV_1 < OBV_break && price_0 < price_1 && price_1 < price_2) bool seller = true;
                  ...
                  if(buyer)
                    {
                     ...
                     buy = OrderSend(Symbol(),buy_order,lot,buy_limit,0,stop_trade,0,NULL,CLT,0,clrNONE);
                    }
                  if(seller)
                    {
                     if(limits)
                     ...
                     sell = OrderSend(Symbol(),sell_order,lot,sell_limit,0,stop_trade,0,NULL,CLT,0,clrNONE);
                    }
                 }
              }
           }
      ...
      for(i = 0; i < OrdersTotal(); i++)
        {
         if(OrderSelect(i,SELECT_BY_POS,MODE_TRADES))
           {
            if(OrderSymbol() == Symbol() && OrderMagicNumber() ==CLT)
              {
