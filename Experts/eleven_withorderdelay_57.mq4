...
input short fast_moving_average_type = 0; //0 = simple, 1 = exponential, 2 = smoothed, 3 = linear
double pips;
      ...
      if(!stops)
        {
         if(!time_randomizer)
           {
            if((price_1 > Bollinger_SD1_Upper && price_1 < Bollinger_SD3_Upper && MA_fast > MA_standard) /*|| (price_1 < Bollinger_SD3_Lower && price_5min_0 > price_1 && Vol_5min_0 > (Vol_timeframe_1/(timeframe/10)))*/)
              {
               buy = OrderSend(Symbol(),OP_BUY,lot,Ask,0,0,0," ",eleven,0,clrNONE);
              }
            if((price_1 < Bollinger_SD1_Lower && price_1 > Bollinger_SD3_Lower && MA_fast < MA_standard) /*|| (price_1 > Bollinger_SD3_Upper && price_5min_0 < price_1 && Vol_5min_0 > (Vol_timeframe_1/(timeframe/10)))*/)
              {
