...
extern short hour_begin = 10, hour_end = 15, timeframe = 240, stop_loss = 30, limit_entry = 30, fast_period = 8, slow_period = 10, signal_period = 3;
...
void OnTick()
  {
   ...
   double ATR_0 = iATR(Symbol(),timeframe,slow_period,0);
   double ATR_1 = iATR(Symbol(),timeframe,slow_period,1);
   double ATR_2 = iATR(Symbol(),timeframe,slow_period,2);
   ...
   if(weekend_breaker)        //breaker is for testing only
     {
      if(!trade_allowed && !paper_buy) 
        {
         if(price_0 > price_1 && price_1 > price_2 && MACD_0 > MACD_1 && MACD_1 > MACD_2 && OBV_0 > OBV_1 && OBV_1 > OBV_2)
           {
            paper_buy = true;
               ...
               if(price_0 > price_1 && price_1 > price_2 && MACD_0 > MACD_1 && MACD_1 > MACD_2 && OBV_0 > OBV_1 && OBV_1 > OBV_2)
                 {
                  ...
                  buy = OrderSend(Symbol(),buy_order,lot,buy_limit,0,stop_trade,0,NULL,truck,0,clrNONE);
                 }
               if(price_0 < price_1 && price_1 < price_2 && MACD_0 < MACD_1 && MACD_1 < MACD_2 && OBV_0 < OBV_1 && OBV_1 < OBV_2)
                 {
                  if(limits)
                  ...
                  sell = OrderSend(Symbol(),sell_order,lot,sell_limit,0,stop_trade,0,NULL,truck,0,clrNONE);
                 }
              }
           }
        }
      ...
      for(i = 0; i < OrdersTotal(); i++)
        {
         if(OrderSelect(i,SELECT_BY_POS,MODE_TRADES))
           {
            if(OrderSymbol() == Symbol() && OrderMagicNumber() == truck)
              {
               if(OrderType() == OP_BUY && price_1 < price_2 && MACD_0 < MACD_1 && MACD_1 < MACD_2 && OBV_0 < OBV_1 && OBV_1 < OBV_2)
                 {
               ...
               if(OrderType() == OP_SELL && price_1 > price_2 && MACD_0 > MACD_1 && MACD_1 > MACD_2 && OBV_0 > OBV_1 && OBV_1 > OBV_2)
                 {
