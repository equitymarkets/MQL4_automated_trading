      ...
      if(MA_low > MA_high)
        {
         if(iRSI(Symbol(),timeframe,RSI_period,PRICE_CLOSE,1) < RSI_bottom)
           ...
           {
            confidence_level = confidence_level + 50;
         ...
         if(OrderSelect(i,SELECT_BY_POS,MODE_TRADES))
           {
            if(OrderSymbol() == Symbol() && OrderMagicNumber() == agg)
              {
               if((OrderType() == OP_BUY) && confidence_level < 25 && iRSI(Symbol(),0,RSI_period,PRICE_CLOSE,1)>(RSI_middle + middle_adj))
                 {
                  buy_close = OrderClose(OrderTicket(),lot,Bid,0,clrNONE);
                 }             
               if((OrderType() == OP_SELL) && confidence_level > -25 && iRSI(Symbol(),0,RSI_period,PRICE_CLOSE,1)<(RSI_middle - middle_adj))
                 {
