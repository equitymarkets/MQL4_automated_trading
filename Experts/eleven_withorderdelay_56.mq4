...
input double lot = .1, volume_adjuster = 2;
input short short_period = 8, timeframe = 240, period = 14, bands_shift = 0, minDelay = 60;
input bool time_randomizer = false, stops = false;
input int stop_loss = 30;
input short fast_moving_average_type = 2; //0 = simple, 1 = exponential, 2 = smoothed, 3 = linear
input short little_volume = 10;
double pips;
...
void OnTick()
  {
   ...
   double price_5min_0 = iClose(Symbol(),5,0);
   ...
   double Vol_little_0 = iVolume(Symbol(),little_volume,0); //1,5,15,30,60,240 chart timeframes
   ...
   double MA_fast = iMA(Symbol(),timeframe,short_period,bands_shift,fast_moving_average_type,PRICE_CLOSE,1);
   double buy, sell;
   int i;
      ...
      if(!stops)
        {
         if(!time_randomizer)
           {
            if((price_1 > Bollinger_SD1_Upper && price_1 < Bollinger_SD3_Upper && MA_fast > MA_standard && Vol_little_0 > (Vol_timeframe_1/timeframe/(little_volume*volume_adjuster))) /*|| (price_1 < Bollinger_SD3_Lower && price_5min_0 > price_1 && Vol_5min_0 > (Vol_timeframe_1/(timeframe/10)))*/)
              {
               buy = OrderSend(Symbol(),OP_BUY,lot,Ask,0,0,0," ",eleven,0,clrNONE);
              }
            if((price_1 < Bollinger_SD1_Lower && price_1 > Bollinger_SD3_Lower && MA_fast < MA_standard && Vol_little_0 > (Vol_timeframe_1/timeframe/(little_volume*volume_adjuster))) /*|| (price_1 > Bollinger_SD3_Upper && price_5min_0 < price_1 && Vol_5min_0 > (Vol_timeframe_1/(timeframe/10)))*/)
              {
         ...
         if(OrderSelect(i,SELECT_BY_POS,MODE_TRADES) == true)
           {
            if(OrderSymbol() == Symbol() && OrderMagicNumber() == eleven)
              {
               if(OrderType() == OP_BUY)
                  if((price_1 > Bollinger_SD3_Upper) || (price_1 > Bollinger_SD1_Lower && price_1 < MA_standard)) 
                 {
                  double buy_close = OrderClose(OrderTicket(),OrderLots(),Bid,0,clrNONE);
                 }
               if(OrderType() == OP_SELL)
                 if((price_1 < Bollinger_SD3_Lower) || (price_1 < Bollinger_SD1_Upper && price_1 > MA_standard)) 
                 {
