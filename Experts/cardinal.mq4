            ...
            if(price_0 > price_1 && price_1 > price_2)
              {
               ...
               buy = OrderSend(Symbol(),buy_order,lot,buy_limit,0,stop_trade,0,NULL,cardinal,0,clrNONE);
              }
            if(price_0 < price_1 && price_1 < price_2)
              {
               if(limits)
               ...
               sell = OrderSend(Symbol(),sell_order,lot,sell_limit,0,stop_trade,0,NULL,cardinal,0,clrNONE);
              }
           }
        }
     }
      ...
      for(i = 0; i < OrdersTotal(); i++)
        {
         if(OrderSelect(i,SELECT_BY_POS,MODE_TRADES))
           {
            if(OrderSymbol() == Symbol() && OrderMagicNumber() == cardinal)
              {
