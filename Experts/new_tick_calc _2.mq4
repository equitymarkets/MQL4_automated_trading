...
extern bool time = false, random = false, stops = false, limits = false, 
            weekend_breaker = false, atv = false;
...
int OnInit()
  {
   ...
   pips = (tick == .00001 || .001 ? (tick * 10) : tick);
   Comment("EA.ex4");
            ...
            if(!random || MarketInfo(Symbol(),MODE_TIME) >= (iTime(Symbol(),timeframe,0) + random_trade_time))
              {
               if(!time || (Hour() >= hour_begin && Hour() <= hour_end)) 
                 {
                  if((buyer && !daytrade) || (buyer && daytrade && Hour() != 23))
                    {
                     if(limits) 
                       { 
                        buy_order = OP_BUYLIMIT;
                        buy_limit = (limit_edge ? (iLow(Symbol(),timeframe,iLowest(Symbol(),timeframe,MODE_LOW,10,0)) + (limit_edge_adj * pips))
                                                  : buy_limit = Ask - (limit_entry * pips));
                       }
                     if(stops) 
                       {
                        stop_trade = (stop_extremes ? (iLow(Symbol(),timeframe,iLowest(Symbol(),timeframe,MODE_LOW,10,0)) - (stop_extremes_adj * pips))
                               : Ask - (stop_loss * pips));
                       }
                     buy = OrderSend(Symbol(),buy_order,lot,buy_limit,0,stop_trade,0,NULL,ea,0,clrNONE);
                    }
                  if((seller && !daytrade) || (seller && daytrade && Hour() != 23))
                    {
                     if(limits)
                       {
                        sell_order = OP_SELLLIMIT;
                        sell_limit = (limit_edge ? (iHigh(Symbol(),timeframe,iHighest(Symbol(),timeframe,MODE_HIGH,10,0)) - (limit_edge_adj * pips))
                                     : sell_limit = Bid + (limit_entry * pips));
                       }
                     if(stops)
                       {
                        stop_trade = (stop_extremes ? (iHigh(Symbol(),timeframe,iHighest(Symbol(),timeframe,MODE_HIGH,10,0)) + (stop_extremes_adj * pips))
                                     : sell_limit = Bid + (stop_loss * pips));
                       }
