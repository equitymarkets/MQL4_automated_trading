...
#define mayday 050122

extern double lot = .5;
extern short hour_begin = 4, hour_end = 23, timeframe = 240, stop_loss = 125, limit_entry = 30;
                  ...
                  if(trends_buy)
                    {
                     ...
                     buy = OrderSend(Symbol(),buy_order,lot,buy_limit,0,stop_trade,0,NULL,mayday,0,clrNONE); 
                    }
                  if(!CheckOpenOrders())
                    {
                     if(trends_sell)
                       {
                        ...
                        sell = OrderSend(Symbol(),sell_order,lot,sell_limit,0,stop_trade,0,NULL,mayday,0,clrNONE); 
                       }
                    }
                 }
      ...
      for(i = 0; i < OrdersTotal(); i++)
        {
         if(OrderSelect(i,SELECT_BY_POS,MODE_TRADES))
           {
            if(OrderSymbol() == Symbol() && (OrderMagicNumber() == mayday || OrderMagicNumber() == 113018))
              {
