...
input short volume_value = 5, MA_value = 5, price_value = 5, RSI_value = 5, confidence_breakpoint = 20;
input short ma_method = 0, RSI_period = 14;
input double volume_multipier = 2.5;                //multiply by existing volume quotient
double pips;
int i;
...
void OnTick()
  {
   ...
   int confidence_level = 0;
      ...
      //5
      if(volume_short_1 > (volume_long_1/24 * volume_multipier))
        {
      ...
      if(price_short_1 > price_short_2)
        {
         confidence_level = confidence_level + price_value;
        }
      if(price_short_1 > price_short_2 && volume_short_1 > volume_short_2)
        {
         confidence_level = confidence_level + volume_value;
        }
      ...
      if(price_mid_1 > price_mid_2)
        {
         confidence_level = confidence_level + price_value;
        }
      if(price_mid_1 > price_mid_2 && volume_mid_1 > volume_mid_2)
        {
         confidence_level = confidence_level + volume_value;
        }
      ...
      if(price_short_1 > fastMA_short_1)
        {
         confidence_level = confidence_level + MA_value;
        }
      if(price_short_1 < fastMA_short_1)
   ...
   if(!CheckOpenOrders())
     {
      if(confidence_level >= confidence_breakpoint && (volume_mid_0 > (((volume_long_1 + volume_long_2 + volume_long_3)/18)*1.2) && (price_mid_1 > fastMA_mid_1)))
        {
         double buy_order = OrderSend(Symbol(),OP_BUY,lot,Ask,0,0,0," ",layers_new,0,clrNONE);
        }
      if(confidence_level <= -confidence_breakpoint && (volume_mid_0 > (((volume_long_1 + volume_long_2 + volume_long_3)/18)*1.2) && (price_mid_1 < fastMA_mid_1)))
        {
         double sell_order = OrderSend(Symbol(),OP_BUY,lot,Bid,0,0,0," ",layers_new,0,clrNONE);
        }
     }
   if(CheckOpenOrders())
     {
         ...
         if(OrderSelect(i,SELECT_BY_POS,MODE_TRADES))
           {
            if(OrderSymbol() == Symbol() && OrderMagicNumber() == layers_new)
              {
               if ((OrderType() == OP_BUY) && confidence_level < (confidence_breakpoint/2.5))
                 {
                  double sell_close = OrderClose(OrderTicket(),OrderLots(),Bid,0,clrNONE);
                 }
               if((OrderType() == OP_SELL) && confidence_level > -(confidence_breakpoint/2.5))
                 {
