...
extern ushort hour_begin = 0, hour_end = 11, timeframe = 240, stop_loss = 100, limit_entry = 30;
...
double ATV(short atr_timeframe, short obv_timeframe, short atr_period)
  {
...
void OnTick()
  {
   ...
   if((MACD_1 < MACD_2) && Minute() >= what_minute) bool buy_closer = true;
   if((MACD_1 > MACD_2) && Minute() >= what_minute) bool sell_closer = true;
            ...
            if(OrderSymbol() == Symbol() && OrderMagicNumber() == oppo)
              {
               if(OrderType() == OP_BUY && buy_closer)
                 {
                  if(ATV(ATR_timeframe,OBV_timeframe, ATR_period) < ATV_break)   
                    {
