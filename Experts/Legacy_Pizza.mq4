...
input int MA_fast = 13, MA_mid = 21, MA_slow = 34, time_1 = 60, day_set_one = 0, day_set_two = 1;
...
bool Legacy() 
  {
   if(DayOfWeek() == day_set_one || DayOfWeek() == 1)
     {
      return(false);
     }
   return(true);
      ...
      if(trade_allowed)
        {
         if((close_price1 > close_price2) && (ima1 >= ima2) && (close_price1 >= ima1))
           {
            actual_buy_open = OrderSend(Symbol(),OP_BUY,lot,Ask,0,0,0,NULL,pizza,0,clrNONE);
            
           }
         if((close_price1 < close_price2) && (ima1 <= ima2) && (close_price1 <= ima1))
           {
            actual_sell_open = OrderSend(Symbol(),OP_SELL,lot,Bid,0,0,0,NULL,pizza,0,clrNONE);
           }
        }
     }
      ...
      for(int i=0; i<OrdersTotal(); i++) 
        {
         if(OrderSelect(i,SELECT_BY_POS,MODE_TRADES)==true) 
           {
            if(OrderSymbol() == Symbol() && OrderMagicNumber() == pizza)
              {
