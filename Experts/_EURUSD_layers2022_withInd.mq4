...
void sma()
  {
   ...
   double sum = 0;
   int   j, pos=Bars-ExtCountedBars-1;
//--- initial accumulation
   if(pos < VMA_Period) pos = VMA_Period;
   for(j = 1; j < VMA_Period; j++, pos--)
     {
      sum += iVolume(Symbol(), 0, pos);
     }
//--- main calculation loop
   while(pos>=0)
     {
      sum += iVolume(Symbol(),0,pos);
      ExtMapBuffer[pos] = sum/VMA_Period;
      sum -= iVolume(Symbol(),0,pos+VMA_Period-1);
      pos--;
     }
//--- zero initial bars
   if(ExtCountedBars < 1)
     {
      for(j = 1; j < VMA_Period; j++) ExtMapBuffer[Bars-j] = 0;
     }
  }
void ema()
  {
   double pr = 2.0/(VMA_Period+1);
   int    pos = Bars - 2;
   if(ExtCountedBars>2) pos = Bars - ExtCountedBars - 1;
//--- main calculation loop
   while(pos >= 0)
     {
      if(pos == Bars - 2) ExtMapBuffer[pos + 1] = iVolume(Symbol(),0,pos+1);
      ExtMapBuffer[pos] = iVolume(Symbol(), 0, pos) * pr + ExtMapBuffer[pos+1] * (1 - pr);
      pos--;
     }
  }
//34567891123456789212345678931234567894123456789512345678961234567897123456789812345678991234567891
...
void OnDeinit(const int reason)
  {
   Alert("EA layers2022b disconnected from chart!");
  }
