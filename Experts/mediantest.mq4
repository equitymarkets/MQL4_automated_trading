...
void OnTick()
  {
   ...
   double MACD_0 = iMACD(Symbol(),timeframe,fast_period,slow_period,signal_period,PRICE_MEDIAN,MODE_SIGNAL,0);
   double MACD_1 = iMACD(Symbol(),timeframe,fast_period,slow_period,signal_period,PRICE_MEDIAN,MODE_SIGNAL,1);
   double MACD_2 = iMACD(Symbol(),timeframe,fast_period,slow_period,signal_period,PRICE_MEDIAN,MODE_SIGNAL,2);
   ...
   if((MACD_1 > MACD_2) && Minute() >= what_minute) bool buyer = true;
   if((MACD_1 < MACD_2) && Minute() >= what_minute) bool seller = true;
   if(price_1 < price_2 && (MACD_1 < MACD_2) && Minute() >= what_minute) bool buy_closer = true;
   if(price_1 > price_2 && (MACD_1 > MACD_2) && Minute() >= what_minute) bool sell_closer = true;
