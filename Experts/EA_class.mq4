...
#define classesEA 071822                //Change to the magic number
...
//extern int legacy_alternative_lookback = 2000;
int i;
...
void OnTick()
  {
   ...
   PA.price_0 = iClose(Symbol(),timeframe,0);
   PA.price_1 = iClose(Symbol(),timeframe,1);
   PA.price_2 = iClose(Symbol(),timeframe,2);
   ...
   if(PA.price_0 > PA.price_1 && PA.price_1 > PA.price_2) bool buyer = true;
   if(PA.price_0 < PA.price_1 && PA.price_1 < PA.price_2) bool seller = true;
   if(PA.price_1 < PA.price_2) bool buy_closer = true;
   if(PA.price_1 > PA.price_2) bool sell_closer = true;
            ...
            if(!paper_buy) 
            ...
            else
              {
               if(buy_closer)                        //paper close
                 {
                  paper_buy = false;
                  if(PA.price_1 > paper_buy_price)             //profit?
                    {
                     trade_allowed = true;
                    }
                 }
            ...
            if(!paper_sell) 
            ...
            else
              {
               if(sell_closer)                        //paper close
                 {
                  paper_sell = false;
                  if(PA.price_1 < paper_sell_price)            //profit?
                    {
                     trade_allowed = true;
                    }
                 }
                  ...
                  if(buyer)
                    {
                     ...
                     buy = OrderSend(Symbol(),buy_order,lot,buy_limit,0,stop_trade,0,NULL,classesEA,0,clrNONE);
                    }
                  if(seller)
                    {
                     if(limits)
                     ...
                     sell = OrderSend(Symbol(),sell_order,lot,sell_limit,0,stop_trade,0,NULL,classesEA,0,clrNONE);
                    }
                 }
              }
           }
      ...
      for(i = 0; i < OrdersTotal(); i++)
        {
         if(OrderSelect(i,SELECT_BY_POS,MODE_TRADES))
           {
            if(OrderSymbol() == Symbol() && OrderMagicNumber() == classesEA)
              {
