...
extern short CCI_timeframe = 60, CCI_period = 14, fast_period = 8, slow_period = 10, signal_period = 3;
...
void OnTick()
  {
   ...
   double CCI_0 = iCCI(Symbol(),CCI_timeframe,CCI_period,PRICE_CLOSE,0);
   double CCI_1 = iCCI(Symbol(),CCI_timeframe,CCI_period,PRICE_CLOSE,1);
   double CCI_2 = iCCI(Symbol(),CCI_timeframe,CCI_period,PRICE_CLOSE,2);
   double CCI_3 = iCCI(Symbol(),CCI_timeframe,CCI_period,PRICE_CLOSE,3);
         ...
         if(!random || MarketInfo(Symbol(),MODE_TIME) >= (iTime(Symbol(),timeframe,0) + random_trade_time))
           {
            if(!time || (Hour() >= hour_begin && Hour() <= hour_end)) 
              {
               if(MACD_1 > MACD_2 && CCI_0 > 0 && (CCI_0 + CCI_1)/2 > CCI_2 && CCI_2 > CCI_3 && price_1 > price_2)
                 {
                  if(limits) 
                    { 
                  ...
                  buy = OrderSend(Symbol(),buy_order,lot,buy_limit,0,stop_trade,0,NULL,ozzy,0,clrNONE);
                 }
               if(MACD_1 < MACD_2 && CCI_0 < 0 && (CCI_0 + CCI_1)/2 < CCI_2 && CCI_2 < CCI_3 && price_1 < price_2)
                 {
                  if(limits)
                  ...
                  sell = OrderSend(Symbol(),sell_order,lot,sell_limit,0,stop_trade,0,NULL,ozzy,0,clrNONE);
                 }
              }
           }
        }
      ...
      for(i = 0; i < OrdersTotal(); i++)
        {
         if(OrderSelect(i,SELECT_BY_POS,MODE_TRADES))
           {
            if(OrderSymbol() == Symbol() && OrderMagicNumber() == ozzy)
              {
               if(OrderType() == OP_BUY && MACD_1 < MACD_2 && CCI_0 < 0 && (CCI_0 + CCI_1)/2 < CCI_2 && CCI_2 < CCI_3 && price_1 < price_2)
                 {
               ...
               if(OrderType() == OP_SELL && MACD_1 > MACD_2 && CCI_0 > 0 && (CCI_0 + CCI_1)/2 > CCI_2 && CCI_2 > CCI_3 && price_1 > price_2)
                 {
