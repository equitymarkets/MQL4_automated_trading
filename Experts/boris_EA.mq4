...
int OnInit()
  {
   ...
   Comment("boris_EA.ex4 initialized!");
...
void OnDeinit(const int reason)
  {
   Alert("boris_EA.ex4 deinitialized!");
  }
...
void OnTick()
  {
   ...
   if((price_1 > price_2) && (SD1_hi_0 - SD1_mid_0) > (SD1_hi_1 - SD1_mid_1) && price_1 > SD1_mid_1 && Minute() >= what_minute) bool buyer = true;
   if((price_1 < price_2) && (SD1_mid_0 - SD1_lo_0) > (SD1_mid_1 - SD1_lo_1) && price_1 < SD1_mid_1 && Minute() >= what_minute) bool seller = true;
   if((SD1_mid_0 + SD1_mid_1)/2 < SD1_mid_2 && price_1 < SD1_mid_1 && Minute() >= what_minute) bool buy_closer = true;
   if((SD1_mid_0 + SD1_mid_1)/2 > SD1_mid_2 && price_1 > SD1_mid_1 && Minute() >= what_minute) bool sell_closer = true;
      ...
      for(i = 0; i < OrdersTotal(); i++)
        {
         if(OrderSelect(i,SELECT_BY_POS,MODE_TRADES))
           {
            if(OrderSymbol() == Symbol() && OrderMagicNumber() == boris)
              {
