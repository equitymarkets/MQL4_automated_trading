...
input double lot = .1;
input short short_period = 3, timeframe = 240, period = 14, bands_shift = 0, minDelay = 60;
      ...
      if(!stops)
        {
         if(!time_randomizer)
           {
            if((price_1 > Bollinger_SD1_Upper && price_1 < Bollinger_SD3_Upper) /*|| (price_1 < Bollinger_SD3_Lower && price_5min_0 > price_1 && Vol_5min_0 > (Vol_timeframe_1/(timeframe/10)))*/)
              {
               buy = OrderSend(Symbol(),OP_BUY,lot,Ask,0,0,0," ",eleven,0,clrNONE);
              }
            if((price_1 < Bollinger_SD1_Lower && price_1 > Bollinger_SD3_Lower) /*|| (price_1 > Bollinger_SD3_Upper && price_5min_0 < price_1 && Vol_5min_0 > (Vol_timeframe_1/(timeframe/10)))*/)
              {
