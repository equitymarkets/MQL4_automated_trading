...
#define liquid 010120

extern double lot = .5;
extern short hour_begin = 10, hour_end = 15, confidence_timeframe = 240, gate_timeframe = 15, stop_loss = 30, limit_entry = 30;
extern bool time = false, random = false, stops = false, limits = false, weekend_breaker = false;
extern short what_minute = 0;
extern short ATR_period = 3;
extern double loss_adder = 0;
...
void OnTick()
  {
   ...
   double ATR_now = iATR(Symbol(),confidence_timeframe,1,0);
   ...
   if(ATR_now > ATR_1) bool confidence = true;
            ...
            if(paper_buy) 
              {
               if(buy_closer)                        //paper close
                 {
                  paper_buy = false;
                  if((price_1 - loss_adder) > paper_buy_price)             //big move?
                    {
                     trade_allowed = true;
                    }
                 }
            ...
            if(paper_sell) 
              {
               if(sell_closer)                        //paper close
                 {
                  paper_sell = false;
                  if((price_1 + loss_adder) < paper_sell_price)            //big move?
                    {
                     trade_allowed = true;
                    }
                 }
                  ...
                  if(buyer)
                    {
                     ...
                     buy = OrderSend(Symbol(),buy_order,lot,buy_limit,0,stop_trade,0,NULL,liquid,0,clrNONE);
                    }
                  if(seller)
                    {
                     if(limits)
                     ...
                     sell = OrderSend(Symbol(),sell_order,lot,sell_limit,0,stop_trade,0,NULL,liquid,0,clrNONE);
                    }
                 }
              }
           }
      ...
      for(i = 0; i < OrdersTotal(); i++)
        {
         if(OrderSelect(i,SELECT_BY_POS,MODE_TRADES))
           {
            if(OrderSymbol() == Symbol() && OrderMagicNumber() == liquid)
              {
