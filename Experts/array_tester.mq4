...
extern short atr_timeframe = 5, obv_timeframe = 60, atr_period = 21;
...
void OnTick()
  {
   ...
   double* ATV_array[];           //remove if not needed
   int ATV_array_size = array_lookback_period;
   ArrayResize(ATV_array,array_lookback_period);
   
   for(i = 0; i < array_lookback_period; i++)   //sub any data
     {
      ATV_array[i] = iATR(Symbol(),atr_timeframe,atr_period,i) * iOBV(Symbol(),obv_timeframe,PRICE_CLOSE,i);
     }
