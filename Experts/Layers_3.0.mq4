...
void OnTick()
  {
   ...
   double ima_mid0 = iMA(Symbol(),mid_chart,MA_mid,0,MODE_SMA,PRICE_CLOSE,0);
   double ima_mid1 = iMA(Symbol(),mid_chart,MA_mid,0,MODE_SMA,PRICE_CLOSE,1);
   double ima_mid2 = iMA(Symbol(),mid_chart,MA_mid,0,MODE_SMA,PRICE_CLOSE,2);
         ...
         if((close_price_mid1 > close_price_long) && (ima_fast1 > ima_slow1) && (ima_slow1 > ima_slow2) && (vol_short/short_chart > vol_mid/mid_chart) && (vol_mid/mid_chart > vol_long/long_chart))
           {
            if(close_price_short0 > close_price_short1)
              {
               if(ima_fast1 > ima_mid1)
                {
                  double buy_double = OrderSend(Symbol(),OP_BUY,double_lot,Ask,0,0,0," ",layers,0,clrNONE);
                 }
               else
                 {
                  double buy = OrderSend(Symbol(),OP_BUY,lot,Ask,0,0,0," ",layers,0,clrNONE);
                 }
              }
           }
        } 
            ...
            if(close_price_short0 < close_price_short1)
              {
               if(ima_fast1 < ima_mid1)
                 {
                  double sell_double = OrderSend(Symbol(),OP_SELL,double_lot,Bid,0,0,0,NULL,layers,0,clrNONE);
                 }
               else
                 {
                  double sell = OrderSend(Symbol(),OP_SELL,lot,Bid,0,0,0,NULL,layers,0,clrNONE);
                 }
              }
           } 
        }
