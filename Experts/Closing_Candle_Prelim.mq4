...
extern int in_time = 58;
extern int out_time = 0;
extern double limit_amount = 0;
double pips;
//int i;
//+------------------------------------------------------------------+
            ...
            if(close_price0 > close_price1)   
               if(ima0 >= ima1)
                  if(close_price0 >= ima0)
                    {
                     double trend_buy = OrderSend(Symbol(),OP_BUY,lot,Ask,0,0,0,NULL,0100,TimeCurrent() + 600,clrNONE);
         ...
         //if(ADX0 > ADXbreak)
            if(close_price0 < close_price1)   
               if(ima0 <= ima1)
                  if(close_price0 <= ima0)  
                    {
                     double trend_sell = OrderSend(Symbol(),OP_SELL,lot,Bid,0,0,0,NULL,0100,TimeCurrent() + 600,clrNONE);
                    }  
           }     
