...
input double lot = .1;
input short short_period = 3, timeframe = 240, period = 21, bands_shift = 0;
      ...
      if(!stops)
        {
         if(!time_randomizer)
           {
            if((price_1 > MA_low && price_1 > Bollinger_SD1_Upper && price_1 < Bollinger_SD2_Upper) || (price_1 < Bollinger_SD3_Lower))
              {
               buy = OrderSend(Symbol(),OP_BUY,lot,Ask,0,0,0," ",eleven,0,clrNONE);
              }
            if((price_1 < MA_low && price_1 < Bollinger_SD1_Lower && price_1 > Bollinger_SD2_Lower) || (price_1 > Bollinger_SD3_Upper))
              {
         ...
         if(OrderSelect(i,SELECT_BY_POS,MODE_TRADES) == true)
           {
            if(OrderSymbol() == Symbol() && OrderMagicNumber() == eleven)
              {
               if(OrderType() == OP_BUY)
                  if((price_1 > Bollinger_SD2_Upper && price_1 < Bollinger_SD3_Upper) || (price_1 > Bollinger_SD1_Lower && price_1 < Bollinger_SD1_Upper))
                 {
                  double buy_close = OrderClose(OrderTicket(),OrderLots(),Bid,0,clrNONE);
                 }
               if(OrderType() == OP_SELL)
                 if((price_1 < Bollinger_SD2_Lower && price_1 > Bollinger_SD3_Lower) || (price_1 < Bollinger_SD1_Upper && price_1 > Bollinger_SD1_Lower))
                 {
