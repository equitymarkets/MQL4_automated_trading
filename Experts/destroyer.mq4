//What enables us to NOT be looking at and waiting for a bar close in order to qualify trade?
/*
...
void OnTick()
  {
   ...
   double smoothed_price_0 = iMA(Symbol(),60,3,0,MODE_SMA,PRICE_CLOSE,0);
   double smoothed_price_reference = iMA(Symbol(),60,3,0,MODE_SMA,PRICE_CLOSE,_reference);
   ...
   if(smoothed_price_0 > smoothed_price_reference && MACD_1 > MACD_2) bool buyer = true;
   if(smoothed_price_0 < smoothed_price_reference && MACD_1 < MACD_2) bool seller = true;
   if(smoothed_price_0 < smoothed_price_reference && MACD_1 < MACD_2) bool buy_closer = true;
   if(smoothed_price_0 > smoothed_price_reference && MACD_1 > MACD_2) bool sell_closer = true;
                  ...
                  if(buyer)
                    {
                     ...
                     buy = OrderSend(Symbol(),buy_order,lot,buy_limit,0,stop_trade,0,NULL,destroyer,0,clrNONE);
                    }
                  if(seller)
                    {
                     if(limits)
                     ...
                     sell = OrderSend(Symbol(),sell_order,lot,sell_limit,0,stop_trade,0,NULL,destroyer,0,clrNONE);
                    }
                 }
              }
           }
      ...
      for(i = 0; i < OrdersTotal(); i++)
        {
         if(OrderSelect(i,SELECT_BY_POS,MODE_TRADES))
           {
            if(OrderSymbol() == Symbol() && OrderMagicNumber() == destroyer)
              {
