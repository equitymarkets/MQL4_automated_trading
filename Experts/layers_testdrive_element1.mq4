   ...
   if(!CheckOpenOrders())
     {
      if(!stops)
      ...
      else 
        {
         if(iMA(Symbol(),mid_chart,3,0,ma_method,PRICE_CLOSE,1)>(iMA(Symbol(),long_chart,3,0,ma_method,PRICE_CLOSE,1)))
           {
            buy = OrderSend(Symbol(),OP_BUY,lot,Ask,0,Ask - (stop_loss * pips),0," ",layers_new,0,clrNONE);
           }
         if(iMA(Symbol(),mid_chart,3,0,ma_method,PRICE_CLOSE,1)<(iMA(Symbol(),long_chart,3,0,ma_method,PRICE_CLOSE,1)))
           {
            sell = OrderSend(Symbol(),OP_SELL,lot,Bid,0,Bid + (stop_loss * pips),0," ",layers_new,0,clrNONE);
           }
        }
     }  
      if(CheckOpenOrders())
