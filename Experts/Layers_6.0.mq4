...
bool Randomizer()
  {
   if(MarketInfo(Symbol(),MODE_TIME) >= (iTime(Symbol(),timeframe,0) + ((MathRand()/2.8))))
     {
      return(true);
     }
   return(false);
                  ...
                  //random_long = MathRand()/2.8;
                    {
                     if(MarketInfo(Symbol(),MODE_TIME) >= (iTime(Symbol(),timeframe,0) + random_long))
                       {
                        double long_order = OrderSend(Symbol(),OP_BUY,lot,Ask,0,0,0,NULL,layers,0,clrNONE);
                          {
                           if(long_order > -1)
                             {
                              random_long = MathRand()/2.8;
                  ...
                  //random_short = MathRand()/2.8;
                    {
                     if(MarketInfo(Symbol(),MODE_TIME) >= (iTime(Symbol(),timeframe,0) + random_short))
                       {
                        double short_order = OrderSend(Symbol(),OP_SELL,lot,Bid,0,0,0,NULL,layers,0,clrNONE);
                          {
                           if(short_order > -1)
                             {
                              random_short = MathRand()/2.8;
